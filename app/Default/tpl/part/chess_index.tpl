...
{foreach name=cate from=$categorys key=title item=types}
	...
	{foreach name=type from=$types item=type}
			<li class=""><span class="ico"></span><a href="##" onclick="Menu.chess.pageCatCategory(this)" data="_chess_pageCatCategory&id={$type->id}" title="{$type->title}">{$type->title|truncate:5:'':true}</a></li>
	{/foreach}
		</ul>
{/foreach}
{/if}
...
<div class="chess_content" id="chess_content">
	<div class="body"><span>
